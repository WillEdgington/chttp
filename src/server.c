...
static int should_keep_alive(HttpRequest *req, int conn_default) {
  const char *key = "Connection";
  void *val = hashmap_get(req->headers, &key);
...
// Handles HTTP/1.1 connection
// WARNING: parsed arena allocator will be reset at the end of each request
int chttp_handle_connection(int client_fd, const char *pub_dir,
  ...
  if (setsockopt(client_fd, SOL_SOCKET, SO_RCVTIMEO, &timeout,
                 sizeof(timeout)) < 0) {
    LOG_ERROR("Failed to set SO_RCVTIMEO timeout on client socket.");
    return -1;
  }
  ...
  while (keep_alive) {
    ...
    if (valread < 0) {
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        LOG_INFO(
            "Connection idle timeout reached for client %s. Closing stream",
            client_ip ? client_ip : "UNKNOWN");
      } else {
        LOG_WARN("Socket read error encountered on client connection.");
      }
      break;
    }
    if (valread == 0) {
    ...
    keep_alive = should_keep_alive(req, keep_alive_default);
